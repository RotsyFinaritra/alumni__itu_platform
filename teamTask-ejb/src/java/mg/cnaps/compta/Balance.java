...
public class Balance {
    ...
    public BalanceDetails[] getBalanceDetails(String date1, String date2, String typeCompte, String compteDebut, String compteFin, String exercice) {
        ...
        try {
            ...
            res = (BalanceDetails[])CGenUtil.rechercher(bd,generateQueryBalance(date1,date2,idCompte,isAnalytique));
            Gson gson = new Gson();
            ...
            String[] valInt = {date1, date2, exercice, exercice, idCompte[0], idCompte[1]};

            res = (mg.cnaps.compta.BalanceDetails[]) CGenUtil.rechercher(bd, colInt, valInt, c, "");*/
    ...
    public String generateQueryBalance(String date1,String date2,String[] compte,boolean isAnalytique){
        String query = "WITH latest_entry AS (\n" +
                ...
                "                  JOIN compta_ecriture ce ON ce.id = cs.mere\n" +
                ...
                "         SELECT\n" +
                "             c.compte,\n" +
                "             COALESCE(le.max_daty, DATE '2001-01-01') AS maxDaty,\n" +
                ...
                "         join compta_sous_ecriture cse on cse.mere = ce.id\n" +
                "         WHERE ce.etat >= 11\n" +
                "           AND datecomptable < DATE '"+date1+"'\n" +
                "     ),\n" +
                "     em AS (\n" +
                "         SELECT\n" +
                "             cse.compte::varchar(100) AS compte,\n" +
                "             SUM(cse.debit) AS debit,\n" +
                "             SUM(cse.credit) AS credit\n" +
                "         FROM\n" +
                "             compta_sous_ecriture cse\n" +
                "                 JOIN compta_ecriture ce ON ce.id = cse.mere\n" +
                "         WHERE\n" +
                ...
                "    COALESCE(SUM(ce.debit), 0) + COALESCE(avg(im.debit), 0) AS cumulDebit,\n" +
                "    COALESCE(SUM(ce.credit), 0) + COALESCE(avg(im.credit), 0) AS cumulCredit,\n"+
                "    coalesce(avg(em.debit),0) as debit,\n" +
                "    coalesce(avg(em.credit),0) as credit,\n" +
                "    LEFT(c.compte, 3)::integer AS compte3, \n" +
                "    LEFT(c.compte, 2)::integer AS compte2 \n" +
                "FROM compta_compte c \n" +
                ...
                "         LEFT JOIN ce_filtered ce ON ce.id = cse.mere AND ce.datecomptable > im.maxDaty"+
